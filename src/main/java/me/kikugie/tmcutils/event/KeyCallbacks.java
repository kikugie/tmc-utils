...
public class KeyCallbacks {
    public static void init() {
        Configs.MiscConfigs.ISORENDER_SELECTION.getKeybind().setCallback(new IsorenderSelectionCallback());
        Configs.MiscConfigs.GIVE_FULL_BOX.getKeybind().setCallback(new GiveFullBoxCallback());
    }

    private static class IsorenderSelectionCallback implements IHotkeyCallback {
        @Override
        public boolean onKeyAction(KeyAction action, IKeybind key) {
            if (key == Configs.MiscConfigs.ISORENDER_SELECTION.getKeybind()) {
    ...
    private static class GiveFullBoxCallback implements IHotkeyCallback {
        @Override
        public boolean onKeyAction(KeyAction action, IKeybind key) {
            if (key == Configs.MiscConfigs.GIVE_FULL_BOX.getKeybind()) {
                GiveFullBox.giveBox();
            }
            return false;
        }
